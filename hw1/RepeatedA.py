...
def check_path(maze, path):
    ...
    # check if every position in the path is a '0'
    for position in path:
        if (maze[position[0]][position[1]] == 1):

            # if there's a '1' False and the position so it can update forward_a's grid
            output = [False, (position[0], position[1])]
            return output
        else:
            continue
    # if not, then the path is good!
    output = [True]
    return output
